...
export default function QualityAssessorDashboard() {
  ...
  if (!data) return <div className="loading">Loading quality queue…</div>;

  return (
    <div data-testid="quality-dashboard">
      <section className="role-hero">
        <p className="eyebrow">QUALITY ASSESSOR · {data.assessor.name}</p>
      ...
      <div className="stat-grid">
        <div className="stat" data-testid="stat-awaiting"><div className="stat-icon orange"><ClipboardCheck size={18} /></div><div><span>Awaiting attestation</span><strong>{data.stats.awaitingAttestation}</strong><small>confidence &lt; 90</small></div></div>
        <div className="stat" data-testid="stat-queue"><div className="stat-icon green"><PackageCheck size={18} /></div><div><span>In queue</span><strong>{data.stats.totalInQueue}</strong><small>Active lots</small></div></div>
        <div className="stat" data-testid="stat-attested"><div className="stat-icon blue"><ShieldCheck size={18} /></div><div><span>By me</span><strong>{data.stats.attestedByMe}</strong><small>Attestations</small></div></div>
      </div>

      {flash && <div className="flash" data-testid="quality-flash">{flash}</div>}

      <section className="section-head" style={{ marginTop: 30 }}><div><p className="eyebrow">QUEUE</p><h2>Lots awaiting your attestation</h2></div></section>
      ...
      <div className="lots-grid">
        ...
        {data.queue.length === 0 && <div className="empty-inline" data-testid="qa-empty">No lots in queue right now. Come back after the next harvest window.</div>}
      </div>

      {data.history?.length > 0 && (
        <>
          <section className="section-head" style={{ marginTop: 40 }}><div><p className="eyebrow">MY RECENT ATTESTATIONS</p><h2>{data.history.length} on record</h2></div></section>
